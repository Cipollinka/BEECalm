...
import ArrowIcon from '@/content/icons/arrow.svg';
import Row from '@/components/common/Row';
import LinearGradient from 'react-native-linear-gradient';
import PlayIcon from '@/content/icons/play.svg';
import PauseIcon from '@/content/icons/pause.svg';
...
export default function Meditation({route}: {route: any}) {
      ...
      if (isPlaying) {
        sound.getCurrentTime(seconds => {
          ...
          if (seconds >= duration - 2) {
            setIsPlaying(false);
            nav.navigate(ScreenNames.MEDITATION_END);
          }
  ...
  return (
    <MainWrapper bottomBlock navDisabled={!currentMood}>
      <LinearGradient
